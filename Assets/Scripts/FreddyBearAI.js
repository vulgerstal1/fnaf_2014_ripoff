...
function ItIsSpawning()
{
	...
	else if(spawnPosition==5 && !doorLbutton.GetComponent(DoorButton).closed)
	{
	whoToMove.transform.position = spawnPointFive.transform.position;
	whoToMove.transform.rotation = spawnPointFive.transform.rotation;
	}
...
if(whoToMove.transform.position == spawnPointTwo.transform.position && playerCamera.GetComponent(ShowHideCameraButton).doorRcounter==1)
	...
	if(whoToMove.transform.position == spawnPreAttack.transform.position && !doorRbutton.GetComponent(DoorButton).closed)
	{
	gameOver=false;
	done=true;
...
if(whoToMove.transform.position == spawnPointFive.transform.position && !doorLbutton.GetComponent(DoorButton).closed)
{ youAreDeadTwo=true; }
...
//	{ youAreDeadTwo=true; }
	...
	if(whoToMove.transform.position == spawnPreAttackTwo.transform.position && !doorLbutton.GetComponent(DoorButton).closed)
	{
